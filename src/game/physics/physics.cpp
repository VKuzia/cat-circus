...
QVector2D physics::ellasticCollisionWall(const QVector2D& impulse,
    ...
    QVector2D wall_vector{static_cast<float>(cos(wall_angle)),static_cast<float>(sin(wall_angle))};
    QPair<double,double> normal_wall{-wall_vector.y(), wall_vector.x()};
    double alpha_cos = QVector2D::dotProduct(impulse, wall_vector)/(impulse.length()*wall_vector.length());
    double alpha_sin = sin(acos(alpha_cos) + wall_angle);
    alpha_cos = cos(asin(alpha_sin));
    answer = {static_cast<float>(impulse.x()*alpha_cos), static_cast<float>(impulse.y()*alpha_sin)};
    return answer;
}


QPair<int,int> physics::Advance(QVector2D* vilocity,const QPair<int,int> coordinates) {
    QPair<int,int> answer = {coordinates.first + vilocity->x(), coordinates.second + vilocity->y() + g.y()/2};
